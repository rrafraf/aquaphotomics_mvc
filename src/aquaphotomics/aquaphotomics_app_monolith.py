...
class FigureCollection:
    ...
    def __init__(self, title="", project_root="."): # Add project_root
        """
        Initialize the figure collection.
        
        Args:
            title: Title for the collection window
            project_root: Path to the project root directory for asset loading.
        """
    ...
    def tabbed_tk_window(self):
        ...
        # Use project_root to find assets
        eraser_icon_path = os.path.join(self.project_root, "assets", "images", "eraser.png")
        try:
            self.IMG = ImageTk.PhotoImage(Image.open(eraser_icon_path), master=self.root_window)
        except Exception as e:
            print(f"Error loading eraser icon from {eraser_icon_path}: {e}")
            self.IMG = None # Fallback
...
class AquaphotomicsFigures(FigureCollection):
    ...
    def __init__(self, title="", project_root="."): # Add project_root
        ...
        Args:
            ...
            project_root: Path to the project root directory for asset loading.
        """
        # Pass project_root to the base class constructor
        super().__init__(title, project_root=project_root) 
        ...
        # Call tabbed_tk_window AFTER base class init and plot setup
...
class AquaphotomicsApp(tk.Tk):
    ...
    def __init__(self):
        ...
        # __file__ -> src/aquaphotomics/aquaphotomics_app_monolith.py
        # dirname -> src/aquaphotomics
        # .. -> src
        # .. -> project root
        ...
        # ConfigManager currently loads from '.' which works because run.py is at the root
        ...
        # Load icons using the determined project root
        self.load_icons()
    ...
    def load_icons(self):
        """Load icon images from the assets/images directory using project_root."""
        icons_dir = os.path.join(self.project_root, "assets", "images")
        ...
        for filename in os.listdir(icons_dir):
            if filename.endswith(".ico") or filename.endswith(".png"):
                ...
                try:
                    self.icons[filename] = tk.PhotoImage(file=icon_path)
                except Exception as e:
                    print(f"Error loading icon {filename} from {icon_path}: {str(e)}")
